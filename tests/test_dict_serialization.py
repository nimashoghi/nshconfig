...
def test_from_dict(sample_config_dict):
    ...
    config = TestConfig.from_dict(sample_config_dict)
...
def test_from_dict_invalid():
    ...
    with pytest.raises(ValueError):
        TestConfig.from_dict({"name": "test"})  # missing required field

    with pytest.raises(ValueError):
        TestConfig.from_dict({"name": "test", "value": "not_an_int"})  # wrong type
