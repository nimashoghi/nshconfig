...
def test_up_past_root_names_chain():
    ...
    msg = str(ei.value)
    assert "no ancestor 2 levels up" in msg
    assert "ancestors here: Root > Leaf" in msg


def test_typed_selector_mismatch_names_expected_actual_and_chain():
    class Expected(C.Config):
        width: int = 1

    class Root(C.Config):
        width: int = 2
        value: int = C.interp(lambda c: c.root(Expected).width)

    with pytest.raises(ValidationError) as ei:
        C.finalize(Root.config_draft())
    msg = str(ei.value)
    assert "expected Expected at Root frame" in msg
    assert "ancestors here: Root" in msg
...
def test_self_cycle_is_loud():
    ...
    d = Lvl.config_draft()
    d.a = C.interp(lambda c: c.self().a)
